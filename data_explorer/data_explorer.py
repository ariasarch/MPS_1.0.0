...
def ensure_internal_shapes(A, C):
    A_int = None
    C_int = None
    if A is not None:
        if A.ndim != 3:
            raise ValueError("A must be 3D")
        print(f"[ensure_internal_shapes] A raw: {A.shape} -> transposing to ({A.shape[1]}, {A.shape[2]}, {A.shape[0]})")
        A_int = np.transpose(A, (1, 2, 0))  # (U,H,W) -> (H,W,U) always
        print(f"[ensure_internal_shapes] A final: {A_int.shape}  H={A_int.shape[0]}, W={A_int.shape[1]}, U={A_int.shape[2]}")
    if C is not None:
        if C.ndim != 2:
            raise ValueError("C must be 2D")
        print(f"[ensure_internal_shapes] C raw: {C.shape} -> transposing to ({C.shape[1]}, {C.shape[0]})")
        C_int = C.T  # (T,U) -> (U,T) always
        print(f"[ensure_internal_shapes] C final: {C_int.shape}  U={C_int.shape[0]}, T={C_int.shape[1]}")
    return A_int, C_int
...
class DataExplorerApp(tk.Tk):
    def __init__(self, cache_path: Optional[str] = None, animal: Optional[int] = None,
        ...
        try:
            ...
            icon_path = os.path.join(current_dir, "icons", "neumaierlabdesign.ico")
    ...
    def load_A(self, path: str) -> None:
        ...
        print(f"[load_A] raw shape: {A_raw.shape}, dtype: {A_raw.dtype}")
        print(f"[load_A] min: {A_raw.min():.4f}, max: {A_raw.max():.4f}")
        
        A_int, _ = ensure_internal_shapes(A_raw, None)
        print(f"[load_A] after ensure_internal_shapes: {A_int.shape}")
        print(f"[load_A] interpreted as H={A_int.shape[0]}, W={A_int.shape[1]}, U={A_int.shape[2]}")
    ...
    def load_C(self, path: str) -> None:
        ...
        print(f"[load_C] raw shape: {C_raw.shape}, dtype: {C_raw.dtype}")
        _, C_int = ensure_internal_shapes(None, C_raw)
        self.C = C_int
        self.path_C = path
        print(f"[load_C] stored C shape: {self.C.shape}  U={self.C.shape[0]}, T={self.C.shape[1]}")
        U = self.A.shape[-1] if self.A is not None else self.C.shape[0]
        print(f"[load_C] original_indices will use U={U}")
        if self.original_indices is None or len(self.original_indices) != U:
            self.original_indices = np.arange(U)
        print(f"[load_C] original_indices length: {len(self.original_indices)}")
