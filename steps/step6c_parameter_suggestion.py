    ...
    def _analysis_thread(self, n_components, n_frames, component_source, optimize_memory, selection_method):
        ...
        try:
            ...
            try:
                ...
                sn_spatial = sn_spatial.values if hasattr(sn_spatial, 'values') else np.array(sn_spatial)
                A_matrix = A_matrix.values if hasattr(A_matrix, 'values') else np.array(A_matrix)
                C_matrix = C_matrix.values if hasattr(C_matrix, 'values') else np.array(C_matrix)
