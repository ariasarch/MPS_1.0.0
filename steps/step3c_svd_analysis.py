...
class Step3cSVDAnalysis(ttk.Frame):
    ...
    def create_overview_plots(self, U, S, Vt, A_init):
        ...
        try:
            ...
            # Create a 1x3 grid
            gs = GridSpec(1, 3, figure=self.overview_fig, hspace=0.3, wspace=0.4)
            ...
            # 3. Component overlap map
            ax3 = self.overview_fig.add_subplot(gs[0, 2])
            overlap_map = (A_init > 0).sum('unit_id')
            im = ax3.imshow(overlap_map, cmap='hot')
            ax3.set_title('Component Overlap Map')
            self.overview_fig.colorbar(im, ax=ax3, fraction=0.046, pad=0.04)
    ...
    def _render_background_analysis(self, b, f):
        ...
        try:
            ...
            mask = freqs > 0
            ax4.plot(freqs[mask], spectrum[mask])
