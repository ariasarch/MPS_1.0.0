...
def save_hw_chunks_direct(
    ...
    for start_idx in range(0, total_frames, batch_size):
        ...
        # Time estimate if we have processed at least one batch
        if batch_times:
...
#     # This ensures all metadata and dimension information is properly saved
#     print(f"Creating initial zarr structure with correct dimension info")
#     # Create a minimal dataset with the first few frames for structure
#     mini_array = array.isel(frame=slice(0, min(10, array_shape[0]))).compute()
#     ds = mini_array.to_dataset(name=name)
...
#     # Delete the existing small array
#     del root[name]
...
def get_optimal_chk(
    ...
    for d in dims:
        ncomp = int(arr.sizes[d] / chk_compute[d])
        # SAFETY CHECK ADDED: Ensure ncomp is at least 1
        if ncomp == 0:
            ncomp = 1
        sz = np.array(factors(ncomp)) * chk_compute[d]
        # SAFETY CHECK ADDED: Remove any chunk sizes larger than the dimension
        sz = sz[sz <= arr.sizes[d]]
        # SAFETY CHECK ADDED: If no valid sizes, use compute chunk
        if len(sz) == 0:
            chk_store[d] = min(chk_compute[d], arr.sizes[d])
        else:
            chk_store[d] = sz[np.argmin(np.abs(sz - chk_store_da[d]))]
        # SAFETY CHECK ADDED: Final check to ensure chunk doesn't exceed dimension
    ...
    if 'frame' in dims:
        ...
        if chk_store['frame'] < min_frame_chunk:
            # Recalculate store chunks to be at least minimum
            ncomp = max(1, int(arr.sizes['frame'] / chk_compute['frame']))
            sz = np.array(factors(ncomp)) * chk_compute['frame']
            if len(sz) > 0:
