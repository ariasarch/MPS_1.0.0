...
def custom_arr_optimize(dsk, keys, fast_funcs=FAST_FUNCTIONS, inline_patterns=[],
                        rename_dict=None, rewrite_dict=None, keep_patterns=[]):
    def _split_key(key, rename_dict=None):
        ...
        return "-".join(kls_ft) if kls_ft else kls[0]

    def _key_renamer(keys, max_fused_key_length=120):
        it = reversed(keys)
        first_key = next(it)
        typ = type(first_key)
        if max_fused_key_length:
            max_fused_key_length -= 5
        def _enforce_max(key_name):
            ...
            return key_name
        if typ is str:
            first_name = _split_key(first_key, rename_dict)
            names = {_split_key(k, rename_dict) for k in it}
            names.discard(first_name)
            names = sorted(names)
            names.append(first_key)
            return _enforce_max("-".join(names))
            ...
            return (_enforce_max("-".join(names)),) + first_key[1:]

    def _check_key(key, pat):
        try:
            return bool(re.search(pat, key))
        except TypeError:
            return bool(re.search(pat, key[0]))

    def _check_pat(key, pat_ls):
        return any(_check_key(key, pat) for pat in pat_ls)

    def _inline_pattern(dsk, pat_ls, inline_constants):
        keys = [k for k in dsk.keys() if _check_pat(k, pat_ls)]
        if keys:
            dsk = inline(dsk, keys, inline_constants=inline_constants)
            for k in keys:
                del dsk[k]
            if inline_constants:
                dsk, _ = cull(dsk, set(list(flatten(keys))))
        return dsk

    def _rewrite_key(key, rwdict):
        typ = type(key)
        k = key[0] if typ is tuple else key
    ...
    key_renamer = _key_renamer
    keep_keys = []
    if keep_patterns:
        key_ls = list(dsk.keys())
        for pat in keep_patterns:
            keep_keys.extend([k for k in key_ls if _check_key(k, pat)])
    dsk = darr.optimization.optimize(
        dsk, keys, fuse_keys=keep_keys,
        fast_functions=fast_funcs, rename_fused_keys=key_renamer,
    )
    if inline_patterns:
        dsk = _inline_pattern(dsk, inline_patterns, inline_constants=False)
    if rewrite_dict:
        dsk_old = dsk.copy()
        for key, val in dsk_old.items():
            key_new = _rewrite_key(key, rewrite_dict)
            if key_new != key:
                dsk[key_new] = val
                dsk[key] = key_new
    return dsk
...
def xrconcat_recursive(var, dims):
    if len(dims) > 1:
        var_dict = var if type(var) is dict else {
            tuple([np.asscalar(v[d]) for d in dims]): v for v in var
        }
        try:
        ...
        for idx, v in var_ps.groupby(level=dims[0]):
            ...
            xr_ls.append(xrconcat_recursive(v.to_dict(), dims[1:]))
        return xr.concat(xr_ls, dim=dims[0])
    else:
        var = list(var.values()) if type(var) is dict else var
...
def identify_erroneous_frames(motion, threshold_factor):
    """
    Identify frames with motion significantly different from the average.
    
    Parameters:
    motion (xarray.DataArray): The motion data
    threshold_factor (float): Number of standard deviations to use as threshold
    
    Returns:
    tuple: (erroneous_frames, height_stats, width_stats)
        erroneous_frames is a list of frame numbers
        height_stats and width_stats are dictionaries with 'mean' and 'std' keys
    """
    height_movement = motion.sel(shift_dim='height').values
    width_movement = motion.sel(shift_dim='width').values
    
    height_mean, height_std = np.mean(height_movement), np.std(height_movement)
    width_mean, width_std = np.mean(width_movement), np.std(width_movement)
    
    height_threshold = height_mean + threshold_factor * height_std
    width_threshold = width_mean + threshold_factor * width_std
    
    erroneous_frames = []
    for i, (h, w) in enumerate(zip(height_movement, width_movement)):
        if abs(h) > height_threshold or abs(w) > width_threshold:
            erroneous_frames.append(i)
    
    height_stats = {'mean': height_mean, 'std': height_std}
    width_stats = {'mean': width_mean, 'std': width_std}
    
    return erroneous_frames, height_stats, width_stats

def drop_frames(data, frames_to_drop):
    """
    Drop specified frames from the data.
    
    Parameters:
    data (xarray.DataArray): The data to process
    frames_to_drop (list): List of frame indices to drop
    
    Returns:
    xarray.DataArray: Data with specified frames dropped
    """
    mask = np.ones(data.sizes['frame'], dtype=bool)
    mask[frames_to_drop] = False
    return data.isel(frame=mask)
